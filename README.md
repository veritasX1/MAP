# MAP
1. Introduction 
The Mesh Anonymity Protocol (MAP) defines an identity-free, decentralized and ephemeral mesh communication system. Its goal is to create a fluctuating network that only consists of currently present participants, without any long-term identities, traceability or central instances.
MAP targets environments where classical, server-based or infrastructure-backed networks (Internet, cellular networks, managed Wi-Fi) are not suitable or not desired. It enables anonymous exchange of files and messages between arbitrary devices within the physical range of local radio technologies.

1.1 Goals The main goals of MAP are: 
...
1.2 Non-Goals MAP explicitly does not aim to: 
...
3. Architectural Overview MAP is layered and separates transport, mesh presence, content distribution and optional identity. The architecture is structured as follows:
• Transport layer 
...
4. Protocol Frame (MAP Frame Format) All MAP messages use a compact, common frame format. Headers are kept as small as possible to minimize metadata.

4.1 Generic MAP Frame The generic MAP frame has the following layout (byte-oriented, network byte order):
...
4.2 Flags Field The Flags field is an 8-bit value with the following proposed meaning: 
...
5.1 Presence Announcement (Type = 0x01) Presence messages are sent periodically (e.g. every 2–5 seconds) over all available transports. They inform other nodes that a MAP participant is present and which services it supports in principle.
Payload structure: 
...
5.2 Ephemeral Network 
Tag (ENT) ENT is a 4-byte value that loosely characterizes the mesh group. ENT is not intended to identify individual nodes.
...
6.1 Content and ContentHash Content is an anonymous byte sequence without filename, path or metadata. Each content object is identified by a ContentHash.
Recommended definition: ContentHash = 16-byte truncation of SHAI256( full_content ) 

6.2 Chunking Content is split into fixed-size chunks (e.g. 2048 bytes). The final chunk may be shorter. Chunks are uniquely identified by (ContentHash, ChunkIndex).

6.3 Content Availability (Type = 0x02) Content Availability messages signal which chunks of a specific content are held by a node.
Payload structure: 
• ContentHash (16 bytes) 
• ChunkBitmap (variable length): Bit mask where bit i indicates availability of chunk i.

6.4 Chunk Request (Type = 0x03) Chunk Request frames are used to request a specific chunk of a content object from any node.
Payload structure: 
• ContentHash (16 bytes) 
• ChunkIndex (2 bytes) 

6.5 Chunk Transfer (Type = 0x04) Chunk Transfer frames carry individual chunks.
Payload structure: 
• ContentHash (16 bytes) 
• ChunkIndex (2 bytes) 
• TotalChunks (2 bytes) 
...
7. Ephemeral Identity Layer (EIL) 
...
7.1 Ephemeral User Identifier (EUI) 
...
7.2 Identity Exchange (Type = 0x10) Identity Exchange messages can be used to announce EUI and capabilities (e.g. ready to receive chat messages).
Payload structure: 
• EUI (16 bytes) 
• Capabilities (1 byte, e.g. chatIcapable, broadcastIonly).

8. Portable Content Layer (PCL): 
...
8.1 Portable Content Extraction (PCE) 
...
9. Mesh Naming (Mesh Name Token, MNT) 
...
10.1 Metadata Reduction Implementations MUST ensure that: 
...
11.1 Minimum Requirements (MUST) 
...
11.2 Recommended Behavior (SHOULD) 
